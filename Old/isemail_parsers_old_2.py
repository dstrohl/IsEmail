...
class ParserOps(object):
    ...
    @staticmethod
    def _get_char(data, char_set, min_repeat=1, max_repeat=1, optional=False,
        ...
        try:
            tmp_ret = []
            if len(data.rem_string) < min_repeat:
            ...
            for i in range(len(data.rem_string)):
                ...
                if max_repeat is not None and i > max_repeat-1:
                    log_ddebug('%s found, max_repeat (%s) met!', space_text, max_repeat)
                    break
                ...
                if tmp_next_char in char_set:
                    ...
                    log_ddebug('%s missed, min_repeat (%s) not met!', space_text, min_repeat)
                    raise NoElementError(data, popped_text=tmp_ret)
                else:
                    log_ddebug('%s missed', space_text)
                    break
            if tmp_ret:
                if len(tmp_ret) < min_repeat:
                    log_ddebug('%s missed, min_repeat (%s) not met!', space_text, min_repeat)
                    raise NoElementError(data, popped_text=tmp_ret)
    ...
    def _get_and(self, data, *operations, min_repeat=1, max_repeat=1, optional=False, return_string=True,
        ...
        try:
            for i in range(len(data.rem_string)):
                if max_repeat is not None and i > max_repeat-1 or not data.rem_string:
                    break
    ...
    def _get_repeat(self, data, op, min_repeat=ISEMAIL_MIN_REPEAT, max_repeat=ISEMAIL_MAX_REPEAT, optional=False, return_string=True,
                   on_fail=None, on_pass=None, element_name=None):
        tmp_ret_list = []
        tmp_pos = data.position
        try:
